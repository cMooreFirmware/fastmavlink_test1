...
typedef enum HEADING_TYPE {
    ...
    HEADING_TYPE_ENUM_END = 3,  // end marker
} HEADING_TYPE;
#endif
...
typedef enum MAV_CMD {
    ...
    MAV_CMD_DO_LAND_START = 189,  // Mission item to mark the start of a mission landing pattern, or a command to land with a mission landing pattern.        When used in a mission, this is a marker for the start of a sequence of mission items that represent a landing pattern.        It should be followed by a navigation item that defines the first waypoint of the landing sequence.        The start marker positional params are used only for selecting what landing pattern to use if several are defined in the mission (the selected pattern will be the one with the marker position that is closest to the vehicle when a landing is commanded).        If the marker item position has zero-values for latitude, longitude, and altitude, then landing pattern selection is instead based on the position of the first waypoint in the landing sequence.	      When sent as a command it triggers a landing using a mission landing pattern.	      The location parameters are not used in this case, and should be set to 0.	 | Empty | Empty | Empty | Empty | Latitude for landing sequence selection, or 0 (see description). Ignored in commands (set 0). | Longitude for landing sequence selection, or 0 (see description). Ignored in commands (set 0). | Altitude for landing sequence selection, or 0 (see description). Ignored in commands (set 0).
    ...
    MAV_CMD_DO_SET_STANDARD_MODE = 262,  // Enable the specified standard MAVLink mode.          If the specified mode is not supported, the vehicle should ACK with MAV_RESULT_FAILED.          See https://mavlink.io/en/services/standard_modes.html         | The mode to set. | Reserved (default:0) | Reserved (default:0) | Reserved (default:0) | Reserved (default:0) | Reserved (default:0) | Reserved (default:NaN)
    ...
    MAV_CMD_ENUM_END = 43006,  // end marker
} MAV_CMD;
#endif
...
#ifndef FASTMAVLINK_HAS_ENUM_SECURE_COMMAND_OP
#define FASTMAVLINK_HAS_ENUM_SECURE_COMMAND_OP
typedef enum SECURE_COMMAND_OP {
    ...
    SECURE_COMMAND_OP_ENUM_END = 8,  // end marker
} SECURE_COMMAND_OP;
#endif
...
typedef enum RALLY_FLAGS {
    ...
    RALLY_FLAGS_ENUM_END = 25,  // end marker
} RALLY_FLAGS;
#endif
...
typedef enum EKF_STATUS_FLAGS {
    ...
    EKF_STATUS_FLAGS_ENUM_END = 32769,  // end marker
} EKF_STATUS_FLAGS;
#endif
...
typedef enum PLANE_MODE {
    ...
    PLANE_MODE_ENUM_END = 27,  // end marker
} PLANE_MODE;
#endif
...
typedef enum COPTER_MODE {
    ...
    COPTER_MODE_ENUM_END = 30,  // end marker
} COPTER_MODE;
#endif
...
typedef enum SUB_MODE {
    ...
    SUB_MODE_ENUM_END = 22,  // end marker
} SUB_MODE;
#endif
